...
function LandingPage({ onGetStarted, onViewProcess }) {
  return (
    <div className="flex flex-col">
      ...
      {/* SECTION 2: The Crisis (Problem Focused) - UPDATED IMAGE */}
...
// --- 2. Process Page (Enhanced Human Narrative) ---
function ProcessPage({ onBack, onGetStarted }) {
  return (
    <div className="flex flex-col pb-20 pt-12 bg-[#FDFCF8]">
      ...
      {/* PHASE 5: TRUST (Testimonial) */}
      <section className="px-6 mb-20">
         <Reveal>
             <div className="max-w-4xl mx-auto bg-white border border-emerald-900/10 p-12 rounded-[40px] shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
                   ...
                   <div>
                      ...
                      <p className="text-2xl text-emerald-950 leading-relaxed italic mb-6">
...
// --- 3. Auth Page (Split Screen with Relatable Visual) ---
function AuthPage({ user, setAppUser, setView, appId, showNotification }) {
  ...
  const handleAuth = async (e) => {
    ...
    try {
        ...
        if (!currentUser) {
            ...
            } catch(e) { throw new Error("Connection failed. Please refresh."); }
        }
...
// --- 4. Dashboard ---
function Dashboard({ user, appUser, appId, showNotification }) {
...
function ResidentView({ requests, appUser, appId, showForm, setShowForm, showNotification }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'pickup_requests'), {
        ...formData, 
  ...
  const handleDelete = async (id) => {
    if(confirm("Withdraw request?")) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'pickup_requests', id));
  };
...
function AdminView({ requests, appId }) {
  const updateStatus = async (id, s) => { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'pickup_requests', id), { status: s }); };
